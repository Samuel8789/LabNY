...
addParameter(p,'parProc',true, @(x) islogical(x) && numel(x)==1);
...
addParameter(p,'s_lambda_max',0.05, @(x) isnumeric(x) && numel(x)==1 && x<=1 && x>0);
...
addParameter(p,'num_structures',10,@(x) isnumeric(x) && numel(x)==1 && x>=1);
...
addParameter(p,'p_lambda_count',3,@(x) isnumeric(x) && numel(x)==1 && x>=1);
...
addParameter(p,'sizeEnsCrit','coactUDF',@(x) ischar(x) && (strcmp(x,'max_degree') || strcmp(x,'coact') || strcmp(x,'coactUDF')));
