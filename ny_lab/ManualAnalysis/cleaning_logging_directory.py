...
def clean_up_logging_dir():
    ...
    all_error_logs=glob.glob(os.path.join(path+os.sep+'app_errors**.log'))
    last_log_number = max(all_error_logs, key=os.path.getctime)[55:-4]

    
    alllogs=glob.glob(os.path.join(path+os.sep+'app_**.log'))
    
    nonerror=[ log for log in all_error_logs if os.path.getsize(log)==0]
    
    nonerrorinex= [log[55:-4] for log in nonerror]
    
    logs_to_delete=[glob.glob(os.path.join(path+os.sep+f'app_{i}.log'))[0] for i in nonerrorinex]
    ...
    error_logs=glob.glob(os.path.join(path+os.sep+'app_**.log'))
