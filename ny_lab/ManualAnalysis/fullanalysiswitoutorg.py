# -*- coding: utf-8 -*-
"""
Created on Mon Jan  2 10:47:38 2023
...
tosavedir = r"C:\Users\sp3660\Desktop\TemporaryProcessing"
if not os.path.isdir(tosavedir):
    os.mkdir(tosavedir)
prairireseqdir = r"F:\Projects\LabNY\Imaging\2022\20221213\Mice\SPQU\TestAcquisitions\Aq_3\221213_SPQU_OptoScreen_25x_940_51020_60745_with-001\Ch2Green\plane1"
aqname = (
    os.path.split(os.path.split(os.path.split(prairireseqdir)[0])[0])[1]
    + "_"
    + os.path.split(prairireseqdir)[1]
)
led_start_end_file = aqname + "_LED_start_end.txt"

# %% raw mmap loading
rawmmap = r"D:\Projects\LabNY\Full_Mice_Pre_Processed_Data\Mice_Projects\Interneuron_Optogenetics\G2C\Ai14\SPLJ\imaging\20221018\test aquisitions\221018_SPLJ_10MinTest_25x_940_51020_60745_with-000\planes\Plane1\Green\221018_SPLJ_10MinTest_25x_940_51020_60745_with-000_plane1_Shifted_Movie_d1_256_d2_256_d3_1_order_F_frames_16951_.mmap"

# %% clean up ssh folder
sshpath = r"G:\Projects\TemPrairireSSH\20230328"
recursively_eliminate_empty_folders(sshpath)
...
clipped_mov = cm.load(
    glob.glob(os.path.join(tosavedir, aqname) + "_LED_clipped_d1**.mmap")[0]
)
clipped_mov = movie[movie_start_frame:movie_end_frame]

if not glob.glob(tosavedir + "\**bidiphases**.pkl"):
    bidiphases = []
    ...
    bidiphase_file_path = (
        os.path.join(tosavedir, aqname)
        + f"_bidiphases_maxshift-{bidiphases[0][1]}_2d-sigma-{bidiphases[0][2]}.pkl"
    )
...
if not glob.glob(tosavedir + "\**bidicorrected_d1**.mmap"):
    clipped_bidishifted_mov = np.zeros(clipped_mov.shape).astype("float32")

    for j in range(clipped_mov.shape[0]):
        print("shifitng movie")
        clipped_bidishifted_mov[j, :, :] = shiftBiDi(
            bidiphases[j][0], clipped_mov[j, :, :]
        )

    sigma = 3
    smoothed_clipped_bidishifted_mov = cm.movie(
        sp.ndimage.gaussian_filter1d(clipped_bidishifted_mov, sigma, axis=0)
    )
    smoothed_clipped_mov = cm.movie(
        sp.ndimage.gaussian_filter1d(clipped_mov, sigma, axis=0)
    )

    cm.movie(clipped_bidishifted_mov).save(
        os.path.join(tosavedir, aqname) + "_LED_clipped_bidicorrected.mmap", to32=False
    )
    ...
    smoothed_clipped_mov.save(
        os.path.join(tosavedir, aqname) + f"_LED_clipped_smoothed_{sigma}_sigma.tiff",
        to32=False,
    )
    ...
    smoothed_clipped_bidishifted_mov.save(
        os.path.join(tosavedir, aqname)
        + f"_LED_clipped_bidicorrected_smoothed-{sigma}-sigma.tiff",
        to32=False,
    )
...
fname = glob.glob(
    os.path.join(tosavedir, aqname) + "**_LED_clipped_bidicorrected_d1**.mmap"
)[0]
images = cm.load(
    glob.glob(
        os.path.join(tosavedir, aqname) + "**_LED_clipped_bidicorrected_d1**.mmap"
...
rigid_mc_file = os.rename(
    rigid_mc_file,
    rigid_mc_file[:to_remove] + "_caiman_mc_rigid" + rigid_mc_file[to_keep + 5 :],
)
rigid_registered_mov = cm.load(
    glob.glob(
        os.path.join(tosavedir, aqname)
        + "_LED_clipped_bidicorrected_caiman_mc_rigid_d1**.mmap"
    )[0]
)
cm.movie(sp.ndimage.gaussian_filter1d(rigid_registered_mov, sigma, axis=0)).save(
    os.path.join(tosavedir, aqname)
    + f"_LED_clipped_bidicorrected_caiman_mc_rigid_smoothed-{sigma}-sigma.tiff"
)
...
rigid_mc_file = os.rename(
    elastic_mc_file,
    elastic_mc_file[:to_remove] + "_caiman_mc_elastic" + elastic_mc_file[to_keep + 5 :],
)
elastic_registered_mov = cm.load(
    glob.glob(
        os.path.join(tosavedir, aqname)
        + "_LED_clipped_bidicorrected_caiman_mc_elastic_d1**.mmap"
    )[0]
)
cm.movie(sp.ndimage.gaussian_filter1d(elastic_registered_mov, sigma, axis=0)).save(
    os.path.join(tosavedir, aqname)
    + f"_LED_clipped_bidicorrected_caiman_mc_elastic_smoothed-{sigma}-sigma.tiff"
)
...
moviehandle = cm.concatenate(
    [
        images.resize(1, 1, ds_ratio) - mc.min_mov * mc.nonneg_movie,
        elastic_registered_mov.resize(1, 1, ds_ratio),
    ],
    axis=2,
)
...
bidishifted_mov = cm.load(
    glob.glob(os.path.join(tosavedir, aqname) + "_LED_clipped_bidicorrected_d1**.mmap")[
        0
    ]
)
...
movies = [
    ...
    clipped_mov,
    bidishifted_mov,
    rigid_registered_mov,
    elastic_registered_mov,
]
...
fname_new = cm.save_memmap(
    [movie_to_process], base_name=os.path.join(tosavedir, aqname) + "memmap_", order="C"
)
...
fr = meta.translated_imaging_metadata[
    "FinalFrequency"
...
fr = meta.translated_imaging_metadata[
    "FinalFrequency"
]  # frame rate (Hz) 3pl + 4ms = 15.5455
decay_time = (
    0.06  # 2 for s 0.5 for f # approximate length of transient event in seconds
)
gSig = (3, 3)  # expected half size of neurons
p = 2  # order of AR indicator dynamics
min_SNR = 1.5  # minimum SNR for accepting new components
ds_factor = (
    1  # spatial downsampling factor (increases speed but may lose some fine structure)
)
gnb = 2  # number of background components
gSig = tuple(
    np.ceil(np.array(gSig) / ds_factor).astype("int")
)  # recompute gSig if downsampling is involved
mot_corr = False  # flag for online motion correction
pw_rigid = (
    False  # flag for pw-rigid motion correction (slower but potentially more accurate)
)
