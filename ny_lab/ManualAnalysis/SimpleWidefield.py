...
opts_dict={}
dirpath=r'C:\Users\sp3660\Documents\Projects\LabNY\Amsterdam\20230831'
foldername='31-Aug-2023_8'
...
#     matcontents=io.loadmat(os.path.join(dataPath,f"{foldername}.mat"))
...
if (not alldata.any()) or (not goodtrials.any()):
    ...
    for trialNr in range(nrTrials):
        try:
            ...
            darkframes= np.where(temp < min(temp)*.95)[0] #index for black frames
    ...
    goodtrials=np.delete(np.arange(0,nrTrials),trialstoremove)
    io.savemat(os.path.join(dataPath,f"{foldername}.mat"), {"correcteddata": alldata,"goodtrials":goodtrials})            
...
avgMap = alldata[:,:,stimOn+1:stimOn+60,goodtrials].mean(axis=(2,3))
