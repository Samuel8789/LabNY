...
float thres1 = 0.00005; // threshold for capactititave sensor lick detection (in Volts)
float thres2 = 0.00005; // threshold for capactititave sensor lick detection (in Volts)

float dump; //temp to store biased sensor signal
...
int baselineSamplePeriod = 48; // sampling period for baseline in ms
...
void loop()
{
  ...
  if (runtime - lastBaseSampleTime > baselineSamplePeriod && runtime - lastlick > 50) {
    ...
    //      sensorValue1 += analogRead(PIEZO_PIN1) / 1024.0 * 5.0;
    //      sensorValue2 += analogRead(PIEZO_PIN2) / 1024.0 * 5.0;
    //    }
    //
    //    sensorValue1 = sensorValue1 / sensorsmoothing;
    //    sensorValue2 = sensorValue2 / sensorsmoothing;
    //    dump = analogRead(PIEZO_PIN1);
    baseline1[iBaseline] = analogRead(PIEZO_PIN1) / 1024.0 * 5.0;
    //    dump = analogRead(PIEZO_PIN2);
    baseline2[iBaseline] = analogRead(PIEZO_PIN2) / 1024.0 * 5.0;

    //    baseline1[iBaseline] =  analogRead(PIEZO_PIN1) / 1024.0 * 5.0 - (analogRead(PIEZO_PIN1) / 1024.0 * 5.0 + analogRead(PIEZO_PIN2) / 1024.0 * 5.0)/2;
    //     baseline2[iBaseline] =  analogRead(PIEZO_PIN2) / 1024.0 * 5.0 - (analogRead(PIEZO_PIN1) / 1024.0 * 5.0 + analogRead(PIEZO_PIN2) / 1024.0 * 5.0)/2;
    //
    //    baseline1[iBaseline] = analogRead(PIEZO_PIN1) / 1024.0 * 5.0;
    ////    dump = analogRead(PIEZO_PIN2);
    //    baseline2[iBaseline] = analogRead(PIEZO_PIN2) / 1024.0 * 5.0;
    ...
    // Get the median of the baseline
    //    medBase1 = stats.average(baseline1, 40);
    //    medBase2 = stats.average(baseline2, 40);
...
void checkSensors() {
  ...
  // Check the sensors
  //      dump = analogRead(PIEZO_PIN1);
  //  buf1[iBuffer] = (analogRead(PIEZO_PIN1) + analogRead(PIEZO_PIN2))/2;
  //
  //  / 1024.0 * 5.0) - medBase1);
  //
  dump = analogRead(PIEZO_PIN1);
  buf1[iBuffer] = (analogRead(PIEZO_PIN1) / 1024.0 * 5.0) - medBase1;
  buf1[iBuffer] = abs(buf1[iBuffer]);
  dump = analogRead(PIEZO_PIN2);
  buf2[iBuffer] = (analogRead(PIEZO_PIN2) / 1024.0 * 5.0) - medBase2;
  buf2[iBuffer] = abs(buf2[iBuffer]);

  //  buf2[iBuffer] = (analogRead(PIEZO_PIN2) / 1024.0 * 5.0);

  //  buf2[iBuffer] = ((analogRead(PIEZO_PIN2) / 1024.0 * 5.0) - medBase2);

  //          dump = analogRead(PIEZO_PIN1);
  //
  //  buf1[iBuffer] = abs((analogRead(PIEZO_PIN1) / 1024.0 * 5.0)) ;
  //          dump = analogRead(PIEZO_PIN2);
  //
  //  buf2[iBuffer] = abs((analogRead(PIEZO_PIN2) / 1024.0 * 5.0));

  //        dump = analogRead(PIEZO_PIN2);
  //  buf1[iBuffer] = abs(((analogRead(PIEZO_PIN1) - (analogRead(PIEZO_PIN1) + analogRead(PIEZO_PIN2))/2)/ 1024.0 * 5.0) - medBase1);
  //
  //  buf2[iBuffer] = abs(((analogRead(PIEZO_PIN2) - (analogRead(PIEZO_PIN1) + analogRead(PIEZO_PIN2))/2)/ 1024.0 * 5.0) - medBase2);
  ...
  // Get the median of the baseline
  //  sensorValue1 = stats.median(buf1, sensorsmoothing);
  //  sensorValue2 = stats.median(buf2, sensorsmoothing);

  // Get the median of the baseline
  sensorValue1 = stats.average(buf1, 20);
  sensorValue2 = stats.average(buf2, 20);
  //
  //  This part is to view the value of one sensor and the threshold and see whether the threshold is good:
  ////    Serial.print(sensorValue1 * 500); // Print the voltage.
  //
  //  Serial.print(",");          //seperator
  //    Serial.print(sensorValue2 * 500); // Print the voltage.

  //    Serial.print(",");          //seperator

  //     Serial.print((buf1[iBuffer])*500); // Print the voltage.
  //       Serial.print(",");          //seperator

  Serial.print(sensorValue1 * 10000); // Print the voltage.
  Serial.print(",");          //seperator
  //  Serial.print(buf2[iBuffer] * 10000); // Print the voltage.
  //  Serial.print(",");          //seperator
  Serial.print(sensorValue2 * 10000); // Print the voltage.
  Serial.print(",");
  Serial.println(thres2 * 10000);      // Print the threshold.
  Serial.print(",");          //seperator
