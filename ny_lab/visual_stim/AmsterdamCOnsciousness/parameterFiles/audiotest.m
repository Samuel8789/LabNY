...
Par.bgInt                       = 0;%0.3; %background intensity (dbl, [0 1])
Par.bgInt                       = round(Par.bgInt*255);

%% stimulus params
% Visual
Par.strVisStimType              = 'dot';%'gabor';
%Par.dblSpatialFrequency         = 0.005; %spatial frequency of grating (cycle/degree)was 0.01
%Par.dblSigma                    = 200; %100; %standard deviation of gaussian window
...
Par.bgInt                       = 0;%0.5; %background intensity (dbl, [0 1])
Par.bgInt                       = round(Par.bgInt*255);

Par.strVisStimType              = 'dot';
Par.stimBackInt                 = 1; %background intensity during stimulus
Par.DotInt                      = round(Par.stimBackInt*255);;  % Dot intensity       [0-255]
Par.StimSizeRetinalDegrees      = 35;   % Retinal degrees of Doting disk
...
Par.vecNoiseRange               = [14000 18000];%[8000 22000];
...
%Trials
Par.intBlockSizeVisual          = 0; %60 %Number of trials in a visual block  
Par.intBlockSizeAudio           = 20;%4; %40 %Number of trials in an audio block  
...
Par.intVisualBlocks             = 0; %Number of visual blocks %was 12
Par.intAudioBlocks              = 60;%100; %Number of audio blocks  %was 12
...
Par.RewardSize                  = 8;%4;        %Adapt script to deal with multiple reward size, calibrate beforehand
...
Par.CorrectRewardSize           = 10;%8;        %Reward size upon correct response (in uL)
