function Par = mol_par_ch_det_au(varargin)
...
%% Timing parameters
Par.ExpVariability              = 0.6;      %variablility of random exponential durs 1=no var, 0.5 = a lot
...
Par.PassiveRewardDelay          = 0.9;      %Delay after stim change before reward
...
Par.FractionUnimodalTrials      = 1;
Par.UseBiProbeTrials            = 0; %No change                 Trial code = 'P'
Par.FractionBiProbeTrials       = 0;
...
Par.UseTrialBlocks              = 1;        %Whether to use blocks or pseudorandom
Par.BlockSize                   = 800;      %Number of trials in a block, should be even in habituation phase
Par.intFirstBlock               = 0;        %Visual first block so visual trials only (0=auditory)
...
%Par.AllTrainingStages          = [1        2       3           4               5           ];
Stage.GivePassiveReward         = {1        1       0           0               0           };      %Aid training give reward correct side
Stage.CorrectRewardSize         = {10       6       5           5               5           };      %Reward size upon correct response (in uL) correct side
Stage.RespWinSecs               = {3        2       1           2               1           };      %%Time window for responding after stimulus offset
Stage.TolerateLicksIncorrect    = {1        0       0           0               0           };      %Whether to tolerate incorrect licks during stimulus post change or response window
Stage.UseUniProbeTrials         = {0        1       1           1               1           };      %Unimodal stimulation      Trial code = 'A' or 'V'
Stage.FractionUniProbeTrials    = {0        0.1     0.15        0.15            0.15        };
Stage.vecFreqChange             = {4000     4000    4000        [600 2000 4000] [100 200 600 2000 4000]}; 
