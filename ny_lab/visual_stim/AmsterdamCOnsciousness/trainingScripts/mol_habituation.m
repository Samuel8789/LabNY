function mol_habituation(Par,Objects,sessionData)
    ...
    while boolTaskRunning
        ...
        switch state
            ...
            case 'PrepareNewTrial' %State: prepare config for new trial
                ...
                % Print trial number:
                fprintf('Starting trial %d at %.0f min %2.0f secs: ',intThisTrial,floor(vecTrial.trialStart(intThisTrial)/60),mod(vecTrial.trialStart(intThisTrial),60));
                
                state = 'ResponseWindow';
                vecTrial.respwinStart(intThisTrial)       = toc(refTime);
                vecTrial.leftCorrect(intThisTrial)        = 1;
                vecTrial.rightCorrect(intThisTrial)       = 1;

%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            case 'ResponseWindow' %State: prepare config for new trial
                ...
                if toc(refTime)-vecTrial.respwinStart(intThisTrial) > Par.RespWinSecs; %Window length for response
                    vecTrial.respwinEnd(intThisTrial)       = toc(refTime);
                    vecTrial.correctResponse(intThisTrial)  = 0;
                    vecTrial.noResponse(intThisTrial)       = 1;
                    state = 'EndTrial'; ChecklickStart = toc(refTime);
