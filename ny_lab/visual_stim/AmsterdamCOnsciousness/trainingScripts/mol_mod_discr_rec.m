function mol_mod_discr_rec(Par,Objects,sessionData)
...
%Trial start: Serial Code 'a'
TTL_trialStart    = 'a';
%Trial end: Serial Code 'b'
TTL_trialEnd      = 'b';
%iti start: Serial Code 'c'
TTL_itiStart      = 'c';
%iti end: Serial Code 'd'
TTL_itiEnd        = 'd';
%stim start: Serial Code 'e'
TTL_stimStart     = 'e';
%stim end: Serial Code 'f'
TTL_stimEnd       = 'f';
%respwin start: Serial Code 'g'
TTL_respwinStart  = 'g';
%reswin end: Serial Code 'h'
TTL_respwinEnd    = 'h';
%timeout start: Serial Code 'i'
TTL_timeoutStart  = 'i';
%timeout end: Serial Code 'j'
TTL_timeoutEnd    = 'j';
%stimChange: Serial Code 'k'
TTL_stimChange    = 'k';
%opto start: Serial Code 'l'
TTL_optoStart     = 'l';
%opto end: Serial Code 'm'
TTL_optoEnd       = 'm';
%opto start: Serial Code 'n'
TTL_sessionStart     = 'n';
%opto end: Serial Code 'o'
TTL_sessionEnd       = 'o';
%trial type A: Serial Code 'p'
TTL_trialTypeA     = 'p';
%trial type V: Serial Code 'q'
TTL_trialTypeV     = 'q';
%trial type P: Serial Code 'r'
TTL_trialTypeP     = 'r';
%trial type C:Serial Code 's'
TTL_trialTypeC     = 's';

%Initialize serial communication
arduinoTTL = serial(Par.serialPortStr,9600);
...
%Send event to Neuralynx; Pause necessary between arduinos or something
pause(1); srl_write(arduinoTTL,TTL_sessionStart); pause(Par.TTL_time); 
    ...
    while boolTaskRunning
        ...
        switch state
            ...
            case 'PrepareNewTrial' %State: prepare config for new trial
                ...
                srl_write(arduinoTTL,TTL_trialStart); pause(Par.TTL_time); %Send event to Neuralynx
                vecTrial.trialStart(intThisTrial)   = toc(refTime);
                srl_write(arduinoTTL,TTL_itiStart); pause(Par.TTL_time); %Send event to Neuralynx
                ...
                eval(strcat('srl_write(arduinoTTL,TTL_trialType',vecTrial.trialType{intThisTrial},');')); pause(Par.TTL_time); %Send event to Neuralynx
...
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            case 'ITI' %State: Inter-trial interval
                ...
                if Par.TolerateLicksITI || (~Par.TolerateLicksITI && ~any(vecTrial.lickTime{intThisTrial}))
                    if toc(refTime) - vecTrial.itiStart(intThisTrial) > ITIdur %If ITI is over
                        ...
                        state = 'stimStart';
                        srl_write(arduinoTTL,TTL_itiEnd); pause(Par.TTL_time); %Send event to Neuralynx
            ...
            case 'stimStart' %State: stimulus start
                ...
                if boolAudioStim
                    ...
                    srl_write(arduinoTTL,TTL_stimStart); pause(Par.TTL_time); %Send event to Neuralynx
                end
                ...
                if ~boolAudioStim && ~boolVisualStim
                    srl_write(arduinoTTL,TTL_stimStart); pause(Par.TTL_time); %Send event to Neuralynx
                end
            ...
            case 'StimDuring' %State: keep presenting stimuli and detect response
                ...
                if boolLickDetected && toc(refTime)-vecTrial.stimStart(intThisTrial) > Par.GracePeriod && ~Par.TolerateLicksIncorrect
                    ...
                    if (strcmp(side,'L')&&~vecTrial.leftCorrect(intThisTrial)) || (strcmp(side,'R')&&~vecTrial.rightCorrect(intThisTrial))   %Incorrect
                        ...
                        if Par.UseTimeOut
                            ...
                            srl_write(arduinoTTL,TTL_timeoutStart); pause(Par.TTL_time); %Send event to Neuralynx
                        else
                ...
                if boolstimEnd
                    ...
                    srl_write(arduinoTTL,TTL_stimEnd); pause(Par.TTL_time); %Send event to Neuralynx
            ...
            case 'timeout'
                ...
                if toc(refTime)-vecTrial.timeoutStart(intThisTrial) > Par.TimeOutSecs;
                    ...
                    srl_write(arduinoTTL,TTL_timeoutEnd); pause(Par.TTL_time); %Send event to Neuralynx
                    state = 'EndTrial';
                end
            ...
            case 'EndTrial'
                ...
                srl_write(arduinoTTL,TTL_trialEnd); pause(Par.TTL_time); %Send event to Neuralynx
...
srl_write(arduinoTTL,TTL_sessionEnd); pause(Par.TTL_time); %Send event to Neuralynx
