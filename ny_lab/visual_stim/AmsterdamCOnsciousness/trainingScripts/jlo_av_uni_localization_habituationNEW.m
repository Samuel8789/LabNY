function av_uni_localization_habituation(Par,Objects,sessionData)
% Task description here
%localization but now no errors allowed

% offline change: line 189 + was a -
...
%         Objects.cwObj.update(vecTrial,Par,intThisTrial);
        ...
        elseif keyCode(KbName(Par.strRewardKey))
            Objects.ldObj.giveReward(Par.Stim.CorrectSide(intThisTrial),Par.CorrectRewardSize);
            fprintf('Dispensed reward on the correct side of this trial: %s\n', Par.Stim.CorrectSide(intThisTrial));
        elseif keyCode(KbName(Par.strRewardKeyLeft),Par.CorrectRewardSize)
            Objects.ldObj.giveReward('L');
            fprintf('Dispensed reward on the left side\n');
        elseif keyCode(KbName(Par.strRewardKeyRight),Par.CorrectRewardSize)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            case 'PrepareNewTrial' %State: prepare config for new trial
                ...
                % Reset boolean that stores whether a passive reward was
                % already given
...
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            case 'StimDuring' %State: keep presenting stimuli and detect response
                ...
                if boolVisualStim
                    ...
                    if strcmp(Par.strVisStimType, 'gabor')
                        ...
                        Screen('Flip', Objects.ptrWindow);
                                ...
                                % Transition if correct lick during stimulus--> end stim go
                % to reward consumption state
                if boolLickDetected 
                    if vecTrial.lickSide{intThisTrial}(end) == Par.Stim.CorrectSide(intThisTrial) %correct
                        vecTrial.correct(intThisTrial) = 1;      
                          if ~boolGaveReward %jl
                            Objects.ldObj.giveReward(Par.Stim.CorrectSide(intThisTrial),Par.CorrectRewardSize); % give reward on arduino%jlcopy
                            vecTrial.rewardSide(intThisTrial) = Par.Stim.CorrectSide(intThisTrial);%jlcopy
                            vecTrial.rewardSize(intThisTrial) = Par.RewardSize;%jlcopy
                            vecTrial.rewardTime(intThisTrial) = toc(refTime);%jlcopy
                            boolGaveReward = true;         %jlcopy  
                           end       %jl
                ...
                % Print info if lick on incorrect side jl:and end trial
                if boolLickDetected 
                ...
                if ~Par.boolWaitForResponse && toc(refTime) - vecTrial.StimStart(intThisTrial) > Par.dblSecsStimDur
                    boolStimEnd = true;
                    state = 'EndTrial';
