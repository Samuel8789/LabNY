function mol_ch_det(Par,Objects,sessionData)
        ...
        switch state
            ...
            case 'PrepareNewTrial' %State: prepare config for new trial
                ...
                if boolVisualStim
                    ...
                    vecTrial.visualOriChange(intThisTrial)      = Par.Stim.vecOriChange(intThisTrial);
                ...
                if boolAudioStim
                    ...
                    vecTrial.audioFreqChange(intThisTrial)      = Par.Stim.vecFreqChange(intThisTrial);
                    varaudio     = load_auditory_change(Par,intThisTrial);
                    % Fill the audio playback buffer with the audio data 'varAudio.vecSound':
                    PsychPortAudio('FillBuffer', Objects.ptrAudioPre, varaudio.vecSoundPre);
                    PsychPortAudio('FillBuffer', Objects.ptrAudioPost, varaudio.vecSoundPost);
                end
                ...
                PreChangeDur        = min([Par.StimPreChangeSecs*Par.ExpVariability + exprnd(Par.StimPreChangeSecs-Par.StimPreChangeSecs*Par.ExpVariability) ...
                    Par.StimPreChangeSecs*(1/Par.ExpVariability)^2]);
...
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            case 'stimStart' %State: stimulus start
                ...
                state = 'StimDuringPreChange';

                %% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            case 'StimDuringPreChange' %State: keep presenting stimuli and detect response
                ...
                if ~Par.TolerateLicksPreChange && boolLickDetected
                ...
                if boolstimEnd
                    ...
                    Screen('FillRect',Objects.ptrWindow, Par.bgInt);
                    Screen('Flip', Objects.ptrWindow);
                    % Stop any ongoing audio stimPreChangeDurulus
                    PsychPortAudio('Stop',Objects.ptrAudioPre);
                    vecTrial.stimEnd(intThisTrial) = toc(refTime);
                    boolVisualStim = false; boolAudioStim = false; %This makes sure no stim will appear
                end
                
                % Transition if stimulus pre change is over --> post change stim
                if toc(refTime) - vecTrial.stimStart(intThisTrial) > PreChangeDur
                    
                    if Par.Stim.audiochange(intThisTrial) %Au change trials: change auditory stimuli:
                        PsychPortAudio('Start', Objects.ptrAudioPost, 1, 0, 1);
                        PsychPortAudio('Stop', Objects.ptrAudioPre);
                    end
            ...
            case 'StimDuringPostChange' %State: keep presenting stimuli and detect response
                ...
                if boolVisualStim
                    tStamp = mod((toc(refTime) + dblPhaseRand) * Par.dblSpeed, 1);
                    intThisFrame = ceil(tStamp * Par.intFrameRate);
                    if Par.Stim.visualchange(intThisTrial) %If there is a change, use second stimulus:
                        Screen('DrawTexture',Objects.ptrWindow,vecTex(intThisFrame+60));
                    else %continue prechange stimulus
                        Screen('DrawTexture',Objects.ptrWindow,vecTex(intThisFrame));
                    end
                    Screen('Flip', Objects.ptrWindow);
                end
...
%                if (toc(refTime) - vecTrial.stimStart(intThisTrial)) > (PreChangeDur + Par.StimPostChangeSecs)
%                    boolstimEnd = true;
...
%                end
                ...
                if ~boolLickDetected && (toc(refTime) - vecTrial.stimStart(intThisTrial)) > (PreChangeDur + Par.StimPostChangeSecs); %Window length for response
                    boolstimEnd = true;
                ...
                if boolstimEnd
                    ...
                    PsychPortAudio('Stop',Objects.ptrAudioPre);
                    PsychPortAudio('Stop',Objects.ptrAudioPost);
                    vecTrial.stimEnd(intThisTrial) = toc(refTime);
                    boolVisualStim = false; boolAudioStim = false; %This makes sure no stim will appear
                end
            ...
            case 'RewardConsumption' %State: time window for reward consumption
                ...
                if ~boolstimEnd && (toc(refTime) - vecTrial.stimStart(intThisTrial)) > (PreChangeDur + Par.StimPostChangeSecs)
                    boolstimEnd = true;
                end   
