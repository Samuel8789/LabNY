function dl_task_default_v201702(Par,Objects,sessionData)
% Task description here

%% Preallocate trial struct vec for saving info per trial in struct
%  Pre-allocate the fields that will be used by your task
vecTrial = struct('trialStart', [],'trialEnd',[],'itiStart',[],'itiEnd',[],...
    'preStimStart',[], 'stimStart', [], 'lickTime',{{NaN}},'lickSide',{{''}},...
    'correct',[],'rewardSide','','rewardTime',[],'rewardSize',[],...
    'servoCloseTime',[],'servoFarTime',[]);
...
%         new: Objects.cwObj.update(vecTrial,Par,intThisTrial);
        drawnow;
...
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            case 'StimDuring' %State: keep presenting stimuli and detect response
                ...
                % Transition if premature correct lick --> end stim/reward consumption
                if boolLickDetected 
...
save(sessionData.FullFileName,'Par','vecTrial','sessionData');
if exist(strcat(sessionData.FullFileName,'.mat'),'file')
...
delete(instrfindall); %Important to close serial port connection

%% Show error
fprintf('\nReason for exiting: %s\n',errorMessage.message);
