...
acquisition_name='SessionC_FrameCount_getsecs';
...
for parad_num = 1:numel(ops.paradigm_sequence)
 ...
 % check what paradigm
    ...
    if noise   
         ...
         VBLTimestamp= Screen('Flip', win, VBLTimestamp+ (1-0.5 ) * ops.flipInterval ); 
         full_info{1+parad_num,5}{1+trl,2} =GetSecs();
         for trl=2:ops.paradigm_trial_num(parad_num) 
            ...
            VBLTimestamp= Screen('Flip', win, VBLTimestamp+ (waitframes-0.3 ) * ops.flipInterval,2 );  % start trial 2:end
            if desktop_testing==1
     ...
     elseif movi 
        ...
        Screen('DrawTexture', win,texture(1,1), [],[],[],[],[], isi_color);
        VBLTimestamp=Screen('Flip', win, VBLTimestamp+ (1-0.5 ) * ops.flipInterval,2 );  % start trial 1
        if desktop_testing==1
            session.outputSingleScan(trial_volt);
        end
        full_info{1+parad_num,5}{1+trl,2}=GetSecs(); % start trial 1
        ...
        for frame=1:totalframenumer-1
             ...
             VBLTimestamp=Screen('Flip', win,   VBLTimestamp+ (waitframes - 0.3) * ops.flipInterval,2 );   % frame start 2:end            
             if rem(frame,frame_number)+1==1
                ...
                full_info{1+parad_num,5}{1+trl,5}{1+rem(frame,frame_number)+1,2}=GetSecs();
             end            
        end
    else       
        VBLTimestamp = Screen('Flip', win, VBLTimestamp+ (1-0.5 ) * ops.flipInterval );  % start trial 1
        if desktop_testing==1
            session.outputSingleScan(greyvol);
        end
        full_info{1+parad_num,5}{1+1,2}=GetSecs();
        VBLTimestamp = Screen('Flip', win, VBLTimestamp + (waitframes - 0.5) * ops.flipInterval ); % end trial -1   
        waitframes=1;
    end
    Screen('FillRect', win, isi_color, rect);      
    VBLTimestamp=Screen('Flip', win, VBLTimestamp+ (waitframes-0.5 ) * ops.flipInterval );  %paradigm end 
    full_info{1+parad_num,3}=GetSecs();  %paradigm end 
    if desktop_testing==1
        session.outputSingleScan(0);
        session.outputSingleScan(0);
    end
...
%%
for parad_num=1:numel(ops.paradigm_sequence)
    full_info{1+parad_num,4}=full_info{1+parad_num,3}-full_info{1+parad_num,2}; %paradigm time
    for trl=1:ops.paradigm_trial_num(parad_num)
      if contains(ops.paradigm_sequence{parad_num}, 'Noise')
          ...
          full_info{1+parad_num,5}{1+trl,4}=full_info{1+parad_num,5}{1+trl,3}-full_info{1+parad_num,5}{1+trl,2}; % trial time 
...
%                      full_info{1+parad_num,5}{1+trl,5}{frame,1}=frame-1; 
                ...
                full_info{1+parad_num,5}{1+trl,5}{1+frame,4}=full_info{1+parad_num,5}{1+trl,5}{1+frame,3}-full_info{1+parad_num,5}{1+trl,5}{1+frame,2}; %frame time
