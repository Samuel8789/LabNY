...
%             full_info{parad_num+1,5}{trial+1,5}=cell(frame_number+1,4);
%             full_info{parad_num+1,5}{trial+1,5}(1,:)={'Frame', 'FrameStart', 'FrameEnd', 'FrameTime',};
...
%             full_info{parad_num+1,5}{trl+1,2}=GetSecs();
%             for frame=1:frame_number
%                 full_info{parad_num+1,5}{trl+1,5}{frame+1,2}=GetSecs();
%                 frame_volt=9*frame/frame_number;
% %                 session.outputSingleScan(frame_volt);
%                 start_frame=GetSecs();
%                 now=GetSecs(); 
%                 if frame==1 && trl==1
%                      Screen('DrawTexture', win, texture(1,frame));
%                      Screen('Flip',win);
%                      Screen('DrawTexture', win, texture(1,frame+1));
%                      while (now-start_frame)<ops.paradigm_stim_time(parad_num)
%                         now=GetSecs();
%                      end
%                      full_info{parad_num+1,5}{trl+1,5}{frame+1,3}=GetSecs();
%                      Screen('Flip',win);
%                 elseif frame>1 && frame<frame_number
%                     Screen('DrawTexture', win, texture(1,frame+1));
%                     while (now-start_frame)<ops.paradigm_stim_time(parad_num)
%                         now=GetSecs();
%                     end 
...
%                     Screen('Flip',win);
%                     
%                 elseif frame==frame_number && trl<ops.paradigm_trial_num(parad_num)
...
%       if contains(ops.paradigm_sequence{parad_num}, 'Movie')
%         for frame=1:ops.paradigm_frame_number(parad_num)
%             full_info{parad_num+1,5}{trial+1,5}{frame+1,4}=full_info{parad_num+1,5}{trial+1,5}{frame+1,3}-full_info{parad_num+1,5}{trial+1,5}{frame+1,2};
%         end
%       end
%     end
% end
