    ...
    if noise
         for trl=1:ops.paradigm_trial_num(parad_num) 
            ...
            if trl==1
                ...
                Screen('DrawTexture', win, texture(1,trl+1), [],[],[],[],[], isi_color); 
                while (now- full_info{1+parad_num,5}{1+trl,2})<ops.paradigm_stim_time(parad_num)
                        now=GetSecs();
                end
                Screen('Flip',win);
                full_info{1+parad_num,5}{1+trl+1,2}=GetSecs();
%                 full_info{1+parad_num,5}{1+trl,3}= full_info{1+parad_num,5}{1+trl,2}
...
%               full_info{1+parad_num,5}{1+trl,3}= full_info{1+parad_num,5}{1+trl,2}              
                ...
                Screen('Flip',win);  
                full_info{1+parad_num,5}{1+trl,3}=GetSecs(); 
            end         
         end     
    elseif movi 
          for trl=1:ops.paradigm_trial_num(parad_num)
                ...
                while (now-full_info{1+parad_num,5}{1+trl,2})<ops.paradigm_stim_time(parad_num)
                        now=GetSecs();
                        Screen('DrawTexture', win,texture(1,rem(ct,frame_number)+1), [],[],[],[],[], isi_color); 
                        Screen('Flip',win,1,1);
                        full_info{1+parad_num,5}{1+trl,5}{ct+2,2}=GetSecs();
                        ct=ct+1;
                end     
                full_info{1+parad_num,5}{1+trl,3}=GetSecs();
          end
    ...
    full_info{1+parad_num,3}=GetSecs();      
 end
