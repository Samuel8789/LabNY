...
zz=ds.seq_log
test=zz['stimulation']
frames_unique=test['frames_unique']
unique_stim=[fram for fram in frames_unique if fram[7]==0]
unique_stim.append(frames_unique[0])
index_to_display=test['index_to_display']

pppp=log_dict['presentation']['displayed_frames']

indexes_displayed=[ frames_unique.index(frame) for frame in pppp   ]




"""
framesunieuqu has 9 elements eqach frame
...
fig, ax = plt.subplots()
# ys=indexes_displayed
ys = zzzz['001_DriftingGratingCircle']['index_to_display']
...
plt.show()
#%%
refresh=mon_refresh_rate
frame_dur=1*1000/refresh# in ms
frames_per_presentation=dgc_block_dur*1000/frame_dur
frames_per_interval=dgc_midgap_dur*1000/frame_dur

50*120+49*60

diffff=np.diff(greater_than_threshold)
zzzz=np.arange(0,len(index_to_display)*frame_dur,frame_dur)
