function squaregrating_Prairie_90vs0(tempfreq,spfreq,sduration,iduration,nsessions)

%squaregrating_Prairie_90vs0(2,0.035,2,5,1)
...
cpd = .2618; % cm per degree when distance between eye and monitor is 15 cm i.e. tand.5 x 15 x 2
...
angles = 90; %[0 90];  %[0 45 90 135 180 225 270 315];
...
comb_all_sessions = zeros(size(all_combs,1),nsessions);
for i = 1:nsessions
    comb_all_sessions(:,i) = all_combs(:,1)';
end
...
for i = 1:nsessions % loop over sessions
    ...
    for j = 1:size(comb_all_sessions,1) % loop over orientations
        ...
        gratingtex = CreateProceduralSquareWaveGrating(win, stimwindow(1), stimwindow(2), offset*[1,1,1,0], [], con);
        Screen('FillRect', win, lum_center);
        Screen('Flip', win);
