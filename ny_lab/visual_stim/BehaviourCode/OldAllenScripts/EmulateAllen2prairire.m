...
repetitions=15;
ops.paradigm_trial_num =[210,1,5,1,10,1,210,1,5,1,210] ;   
...
%%
% ops.paradigm_trial_num =[length(indixfirst),1,2,1,2,length(indixfirst),1,2,1,length(indixfirst)] ;   
% ops.paradigm_stim_time=[2,5,120,5,30,2,5,120,5,2];
...
% 
% param_vos=zeros(numel(ops.paradigm_sequence),1)
% for parad_num = 1:numel(ops.paradigm_sequence)
%     param_vos(parad_num)=(parad_num/numel(ops.paradigm_sequence))+8;
% end
%% FIRST GRATING TIME 10min
for parad_num = 1:numel(ops.paradigm_sequence)
    grating=0;
%     session.outputSingleScan(8); 
%     session.outputSingleScan(8);  
...
%         Screen('FillRect', win, [0 0 0], [reswidth-diodeBox 0 reswidth diodeBox]); %show the diode box in white
        Screen('Flip',win);
        % draw
        start_stim = GetSecs();
        now=GetSecs();
...
%                         Screen('FillRect', win, [255 255 255], [reswidth-diodeBox 0 reswidth diodeBox]); %show the diode box in white
                        Screen('Flip',win);
                        if ops.driftingGrating
                            ct=ct+1;
                        end
                    else
%                         session.outputSingleScan(4.5);
...
%                 session.outputSingleScan(rem(ct,2));
...
%%
pwd2 = fileparts(which('EmulateAllen2prairire.m')); %mfilename
save_path = pwd2;
