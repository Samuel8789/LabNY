# -*- coding: utf-8 -*-
"""
Created on Wed Jun  2 08:17:36 2021
...
class add_imaging_info:
    def __init__(self,  master, imaging_name):
        
        master.geometry("+2555+0")

       
        
        row_labels=['GreenFilter','RedFilter','DichroicBeamsplitter','IsBlockingDichroic','ExcitationWavelength','CoherentPower', 'Comments']
            ...
            for j in range(len(col_labels)): #Columns
                if j==0:
                    ...
                    if i not in [1,2,3,4]:
                        ...
                        var = StringVar()
                        self.b[i].append(ttk.Combobox(master, values=values,textvariable=var, width=30)) # b[i][j]
                        self.b[i][j].grid(row=i+1, column=j)   
